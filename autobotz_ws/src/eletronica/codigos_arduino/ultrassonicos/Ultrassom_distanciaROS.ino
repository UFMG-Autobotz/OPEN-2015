...
rosserial_arduino::ultrassom distancia;

ros::Publisher chatter("/eletronica/ultrassom/", &distancia);
...
void loop()
{
 ...
 if(num_Ultrassonico == 1){ 
   distancia.esq = distance;
 }
 if(num_Ultrassonico == 2){ 
   distancia.frente = distance;
 }
 if(num_Ultrassonico == 3){ 
   distancia.tras = distance;
 }
 delay(50);
 }
