...
/////////////////////////////// filter() ////////////////////////////////
void filter(cv::Mat in_rgb, cv::Mat out)
{
	std::vector<cv::Mat> three_channels;// Vetor para comportar cada os 3 canais, para uso com a funcao split
	Mat Rchannel, Gchannel, Bchannel; // Variáveis que guardarão cada um dos 3 canais da imagem HSV

	////// Divide a imagem RGB em 3 canais
	split(in_rgb, three_channels);   
	...
	merge(three_channels, out);
}

/////////////////////////////// contorno() ///////////////////////////////
void contorno(cv::Mat in_rgb)
{
    ...
    cv::RNG rng(12345);
    cv::findContours(bw, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
    /// Draw contours
...
void findYellowBlocks(const Mat& image, vector< vector<Point> >& squares)
{
	...
	for(size_t i = 0; i < contours.size(); i++)
	{
		...
		if(approx.size() == 4 && fabs(contourArea(Mat(approx))) > 100 && isContourConvex(Mat(approx)))
		{
			...
			// Se os cossenos de todos os ângulos são pequenos (todos os ângulos são ~90 graus) então escreve os
			//vértices na sequência resultante
			if(maxCosine < 0.26)
			{
